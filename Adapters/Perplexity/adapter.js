    ...
    .then((result) => {
      if (!result || !result.ok) {
        ...
        if (result?.reason === "EXTENSION_CONTEXT_INVALIDATED") {
          console.error("[PromptProfile™ Perplexity] Cannot enhance prompt - extension context invalidated. Please reload the page.");
          enhanceTooltipDismissed = true;
          hideEnhanceTooltip();
...
function showUpgradeButtonInTooltip() {
  ...
  if (!enhanceTooltipElement) {
    console.error("[PromptProfile™ Perplexity] Cannot show upgrade button - tooltip element not found");
    return;
  }
  ...
  if (action) {
    ...
    newAction.addEventListener("click", async (e) => {
      console.log("[PromptProfile™ Perplexity] Upgrade button clicked, calling handleStripeCheckout");
      e.preventDefault();
      e.stopPropagation();
