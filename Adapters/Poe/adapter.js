...
console.log("[PromptProfile™] ========== POE ADAPTER LOADING ==========");
...
function nodeInAssistantMessage(node) {
  ...
  // Poe-specific selectors for assistant messages (works across multiple AI models)
  return !!(
    element.closest("[data-poemessage='bot']") ||
    element.closest("[data-message-author-role='bot']") ||
    element.closest(".Message_botMessageBubble__") ||
    element.closest(".Message_message__")?.querySelector(".Message_botMessageBubble__") ||
    element.closest("[class*='botMessage']") ||
    element.closest("[class*='assistant']") ||
    element.closest(".ChatMessage_message__")?.querySelector("[class*='bot']")
  );
}
...
function nodeInComposer(node) {
  const element = AdapterBase.getElementFromNode(node);
  if (!element) return false;
  // Poe-specific selectors for composer/input area (works across models)
  return !!(
    element.closest("textarea") ||
    element.closest("[contenteditable='true']")?.closest(".ChatInput_inputContainer__") ||
    element.closest("[contenteditable='true']")?.closest(".Composer_composer__") ||
    element.closest("[role='textbox']")?.closest(".ChatInput_inputContainer__") ||
    element.closest("[data-testid='composer']") ||
    element.closest(".Composer_textarea__") ||
    element.closest(".ChatInput_textarea__")
  );
}
...
function capturePoeChatHistory(maxMessages = 20) {
  console.log("%c[PromptProfile™ Poe] ========== capturePoeChatHistory CALLED ==========", "color: blue; font-size: 16px; font-weight: bold;");
  console.log("[PromptProfile™ Poe] Current URL:", window.location.href);
  
  const messages = [];
  
  try {
    // Poe-specific selectors (works across multiple AI models on Poe)
    const assistantSelectors = [
      ...
      "[class*='botMessage']",
      "[class*='assistant']",
      ".ChatMessage_message__[class*='bot']"
    ];
    
    const userSelectors = [
      "[data-poemessage='human']",
      "[data-message-author-role='human']",
      ".Message_humanMessageBubble__",
      "[class*='humanMessage']",
      "[class*='user']",
      ".ChatMessage_message__[class*='human']"
    ];
    ...
    for (const selector of assistantSelectors) {
      try {
        ...
        if (found.length > 0) {
          console.log(`[PromptProfile™ Poe] ✓ Found ${found.length} assistant messages with selector: ${selector}`);
          assistantElements = found;
          break;
        }
      } catch (e) {
        console.warn(`[PromptProfile™ Poe] Selector failed: ${selector}`, e);
      }
    }
    ...
    for (const selector of userSelectors) {
      try {
        const found = Array.from(document.querySelectorAll(selector));
        if (found.length > 0) {
          console.log(`[PromptProfile™ Poe] ✓ Found ${found.length} user messages with selector: ${selector}`);
          userElements = found;
          break;
        }
      } catch (e) {
    ...
    for (const { el, role } of allElements) {
      ...
      if (content && content.length > 10) {
        ...
        console.log(`[PromptProfile™ Poe] Added ${role} message: ${content.substring(0, 50)}...`);
      }
    }
    
    console.log(`[PromptProfile™ Poe] Captured ${messages.length} messages`);
    return messages;
  } catch (error) {
    console.error("[PromptProfile™ Poe] Error capturing chat history:", error);
    return [];
  }
}
...
function submitSelectionToSideChat(text) {
  if (selectionAskInFlight) {
    console.log("[PromptProfile™ Poe] Selection ask already in flight, ignoring");
    return;
  }
  
  if (!text || !text.trim()) {
    console.warn("[PromptProfile™ Poe] Empty text selected, cannot submit");
    return;
  }
  
  selectionAskInFlight = true;
  console.log("[PromptProfile™ Poe] Submitting selection to side chat:", text.substring(0, 50));
  
  const chatHistory = capturePoeChatHistory(20);
  ...
  AdapterBase.sendMessage({
    ...
    .then((response) => {
      ...
      if (response?.ok) {
        console.log("[PromptProfile™ Poe] Side chat request successful");
      } else {
        console.warn("[PromptProfile™ Poe] Side chat request failed:", response?.reason);
      }
    })
    .catch((error) => {
      selectionAskInFlight = false;
      console.error("[PromptProfile™ Poe] Side chat request error:", error);
    });
}
...
function locateComposer() {
  console.log("[PromptProfile™ Poe] locateComposer called");
  
  // Poe-specific selectors (works across multiple AI models)
  const selectors = [
    ".ChatInput_textarea__",
    ".Composer_textarea__",
  ...
  for (const selector of selectors) {
    ...
    if (element instanceof HTMLElement) {
      input = element;
      console.log("[PromptProfile™ Poe] Found input with selector:", selector);
      break;
    }
  }

  if (!input) {
    console.warn("[PromptProfile™ Poe] No input found in locateComposer");
    return null;
  }

  const container = input.closest(".ChatInput_inputContainer__") ||
                    input.closest(".Composer_composer__") ||
                    input.closest("form") || 
                    input.closest("div") || 
                    input.parentElement || 
                    document.body;
  
  console.log("[PromptProfile™ Poe] Composer located:", { input, container });
  return { input, container };
}

function init() {
  console.log("[PromptProfile™ Poe] init() called");
  ...
  if (composer) {
    console.log("[PromptProfile™ Poe] Composer found, setting up enhance tooltip");
    setupEnhanceTooltip(composer.input, composer.container);
    ensureDomObserver();
    return true;
  }
  console.warn("[PromptProfile™ Poe] Composer not found in init()");
  ensureDomObserver();
  return false;
}
...
function handleInsertTextMessage(message, sender, sendResponse) {
  try {
    ...
    console.log("[PromptProfile™ Poe] ========== INSERT TEXT REQUEST ==========");
    console.log("[PromptProfile™ Poe] Text to insert:", textToInsert.substring(0, 50) + (textToInsert.length > 50 ? "..." : ""));
    
    if (!textToInsert) {
      console.log("[PromptProfile™ Poe] Insert failed: EMPTY_TEXT");
      sendResponse({ ok: false, reason: "EMPTY_TEXT" });
      return false;
    }

    console.log("[PromptProfile™ Poe] Searching for composer node...");
    const composerNode = findComposerNode();
    console.log("[PromptProfile™ Poe] Composer node found:", !!composerNode);
    
    if (!composerNode) {
      console.log("[PromptProfile™ Poe] Insert failed: NO_COMPOSER_NODE");
      sendResponse({ ok: false, reason: "NO_COMPOSER_NODE" });
      return false;
    }

    console.log("[PromptProfile™ Poe] Calling setComposerText...");
    const success = setComposerText(composerNode, textToInsert);
    console.log("[PromptProfile™ Poe] setComposerText returned:", success);
    
    if (success) {
      console.log("[PromptProfile™ Poe] Insert succeeded!");
      sendResponse({ ok: true });
    } else {
      console.log("[PromptProfile™ Poe] Insert failed: SET_TEXT_FAILED");
      sendResponse({ ok: false, reason: "SET_TEXT_FAILED" });
    }
    return false;
  } catch (error) {
    console.error("[PromptProfile™ Poe] Insert text handler failed", error);
    sendResponse({ ok: false, reason: error?.message ?? "UNKNOWN" });
    return false;
  }
}

console.log("[PromptProfile™ Poe] Registering PROMPANION_INSERT_TEXT handler with AdapterBase");
...
function handleRefineButtonClick(e) {
  ...
  if (!composerNode) {
    console.error("[PromptProfile™ Poe] No composer node found!");
    return;
  }
  ...
  requestPromptEnhancement(promptText)
    ...
    .catch((error) => {
      console.error("PromptProfile™ Poe: refine request threw", error);
      enhanceActionInFlight = false;
      enhanceTooltipDismissed = true;
      hideEnhanceTooltip();
    });
